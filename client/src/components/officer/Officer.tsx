    ...
    <div className="flex h-screen">
      <div className="m-auto border-2 border-lightgray-500 rounded-xl p-3 size-5/6 bg-white flex flex-col justify-between">
        ...
        <div className="pt-6 pb-6 text-center">
          <span className="text-3xl font-bold">Your services: </span>{' '}
          <span className="text-4xl">{services.join(', ')}</span>
          <br />
          <br />
          <span className="text-3xl font-bold">Customer: </span>{' '}
          <span className="text-4xl">{customerId}</span>
        </div>
        ...
        <div className="flex justify-between">
          {!served && (
            <button
              type="button"
              className="border border-indigo-950 rounded-lg p-1 text-xl hover:bg-indigo-950 hover:text-white"
              onClick={() => {
          ...
          {served && (
            <button
              type="button"
              className="border bg-green-500 rounded-lg p-2 text-xl text-white"
            >
              Served!
            </button>
          )}
          <button
            ...
            className="border border-indigo-950 rounded-lg p-1 text-xl hover:bg-indigo-950 hover:text-white"
            onClick={() =>
              props.nextCustomer(props.counterId).then((cId: string) => {
                setCustomerId(cId);
                setServed(false);
              })
            }
